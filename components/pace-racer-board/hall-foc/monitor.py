...
Keys:
    a / b / c   toggle phase A / B / C trace
    d           toggle DQ current panel
    q           quit
"""
...
PP   = 15   # pole pairs — must match kPolePairs in firmware
...
class CurrentPlot(PlotextPlot):
    ...
    def feed(self, ia: Optional[list], ib: Optional[list], ic: Optional[list]) -> None:
        self.plt.clear_data()
        if ia is not None: self.plt.plot(ia, label="A", color="red")
        if ib is not None: self.plt.plot(ib, label="B", color="green")
        if ic is not None: self.plt.plot(ic, label="C", color="blue")
        self.refresh()
...
class Monitor(App):
    ...
    BINDINGS = [
        Binding("a", "toggle_phase('a')", "Phase A"),
        Binding("b", "toggle_phase('b')", "Phase B"),
        Binding("c", "toggle_phase('c')", "Phase C"),
        Binding("d", "toggle_dq",         "DQ panel"),
    ]
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            with Vertical(id="left"):
                ...
                yield Label(f"target  {kTargetVelocityRpm:.1f} RPM", id="target", classes="val")
            ...
            with Vertical(id="right"):
                ...
                yield Static("PHASE CURRENTS", id="phase-head", classes="head")
                yield CurrentPlot(id="current-plot")
                yield Static("DQ CURRENTS", id="dq-head", classes="head")
    ...
    def on_mount(self) -> None:
        ...
        self._show: set[str] = {"a", "b", "c"}  # active phase traces
        self._read()

    def _phase_head(self) -> str:
        labels = {"a": "A", "b": "B", "c": "C"}
        parts = [f"[{'green' if p in self._show else 'red'}]{labels[p]}[/]" for p in "abc"]
        return "PHASE CURRENTS  " + "  ".join(parts)

    def action_toggle_phase(self, phase: str) -> None:
        if phase in self._show:
            self._show.discard(phase)
        else:
            self._show.add(phase)
        self.query_one("#phase-head", Static).update(self._phase_head())

    def action_toggle_dq(self) -> None:
        dq_plot = self.query_one("#dq-plot", DQPlot)
        dq_head = self.query_one("#dq-head", Static)
        dq_plot.display = not dq_plot.display
        dq_head.display = dq_plot.display
    ...
    def on_row_received(self, m: RowReceived) -> None:
        ...
        self.query_one("#current-plot", CurrentPlot).feed(
            list(self._ia) if "a" in self._show else None,
            list(self._ib) if "b" in self._show else None,
            list(self._ic) if "c" in self._show else None,
        )
        if self.query_one("#dq-plot", DQPlot).display:
...
kTargetVelocityRpm = 100.0  # display only — update to match firmware
