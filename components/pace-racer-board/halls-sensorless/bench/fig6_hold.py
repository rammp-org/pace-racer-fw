"""Fig 6 sustained hold at the max-power edge, 150 rpm."""
...
try:
    ...
    time.sleep(7)
    for bv in (2.5, 3.0, 3.4):
        brake.set(bv, settle=2.0, ilim=3.0)
    bv = 3.65
    brake.set(bv, settle=0.5, ilim=3.0)
    print(f"holding {bv} V...", flush=True)
    t0 = time.time(); buf = ""; stop_reason = "50 s complete"; last_row_t = time.time()
    while time.time() - t0 < 50:
        ...
        if not row:
            if time.time() - last_row_t > 5:
                stop_reason = "telemetry silent 5 s"; break
            continue
        ...
        log.append(rec)
        print(f"t={rec['t']:4.0f}s {rec['bus_w']:5.0f} W  iq={rec['iq']:5.1f}  rpm={rec['rpm']:3.0f}  "
              f"T={tq if tq is None else round(tq,1)}  tmax={tmax:.0f}C", flush=True)
        if tmax == tmax and tmax > 70: stop_reason = "temp"; break
        if rec["rpm"] < 120:
            bv -= 0.06; brake.set(bv, settle=0, ilim=3.0)
            print(f"  trim to {bv:.2f} V", flush=True)
        elif rec["rpm"] > 147 and rec["iq"] < 28.5:
            bv += 0.04; brake.set(bv, settle=0, ilim=3.0)
    ws = [r["bus_w"] for r in log]
    if ws:
        peak = max(ws)
        best10 = max(statistics.mean(ws[i:i+10]) for i in range(max(1, len(ws)-9)))
        print(f"\n{stop_reason}: PEAK {peak:.0f} W, best 10 s avg {best10:.0f} W", flush=True)
finally:
    ...
    json.dump({"log": log}, open(os.path.join(DATA, "power150_hold.json"), "w"))
