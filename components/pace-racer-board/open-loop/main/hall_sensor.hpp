...
public:
  ...
  void update(std::error_code &) {
    ...
    if (t1 == 0 || t1 == t0 || (now - t1) > kVelocityTimeoutUs) {
      ...
      if (candidate < kMaxRpm) rpm_ = candidate; // ignore sector-boundary noise spikes
    }
  }
...
private:
  // ponytail: staircase angle (60° elec resolution), interpolate if PID needs smoothing
  ...
  static constexpr float   kMaxRpm            =  500.0f; // reject implausibly fast velocity readings
    ...
    if (sector >= 0 && prev >= 0) {
      ...
      if (delta != 0) {
        ...
        if (delta == 1 || delta == -1) {
          ...
          self->t_prev_.store(self->t_last_.load(std::memory_order_relaxed), std::memory_order_relaxed);
          self->t_last_.store(esp_timer_get_time(), std::memory_order_relaxed);
        }
      }
    }
    if (sector >= 0) self->prev_sector_ = sector;
  }
