...
public:
  ...
  void init() {
    ...
    if (boot_sec >= 0) steps_.store(boot_sec, std::memory_order_relaxed);
  ...
  void update(std::error_code &) {
    ...
    if (dt_s < 0.02f) return; // called faster than expected: keep the window
...
private:
  ...
  static constexpr float   kMaxRpm            = 1500.0f; // ponytail: was 500 — raise above any expected target

  uint8_t read_state() const {
    return static_cast<uint8_t>((gpio_get_level(cfg_.pin_a) << 2) |
                                (gpio_get_level(cfg_.pin_b) << 1) |
                                 gpio_get_level(cfg_.pin_c));
  }
  ...
  static void isr(void *arg) {
    ...
    if (sector < 0) return; // 0b000 / 0b111 — ignore entirely
    ...
    if (prev < 0 || t_last == 0 || (t_now - t_last) > kVelocityTimeoutUs) {
      ...
      self->t_last_.store(t_now, std::memory_order_relaxed); // keep timeout from re-firing constantly
      return;
    }
    ...
    if (delta == 0) return; // same sector, spurious edge
